...
export default class Translator implements vscode.CodeActionProvider, vscode.Disposable {
    private _disposable: vscode.Disposable = Object.create(null);

    private _isEnabled = false;

    private static ConfigProxy = vscode.workspace.getConfiguration().get<string>('http.proxy') || '';

    public static Source = 'Translator';
    private static Config: vscode.WorkspaceConfiguration = vscode.workspace.getConfiguration(Translator.Source);

    private _diagnosticMap: Map<vscode.Diagnostic[]> = {};
    private _diagnostics: vscode.DiagnosticCollection = Object.create(null);

    public static ReplaceOnSuggestion: string = Translator.Source + '.replaceOnSuggestion';
    private _replaceOnSuggestionCommand: vscode.Disposable = Object.create(null);

    public static AddOnSuggestion: string = Translator.Source + '.addOnSuggestion';
    ...
    private replaceWithSuggestion(document: vscode.TextDocument, diagnostic: vscode.Diagnostic, error: string, suggestion: string): any {
    ...
    private addWithSuggestion(document: vscode.TextDocument, diagnostic: vscode.Diagnostic, error: string, suggestion: string): any {
        const editor = vscode.window.activeTextEditor;
        if (editor) {
            let diagnostics: vscode.Diagnostic[] = this._diagnosticMap[document.uri.toString()];
            let index: number = diagnostics.indexOf(diagnostic);

            const anchor = new vscode.Position(diagnostics[index].range.end.line + 1, 0);
            const active = new vscode.Position(diagnostics[index].range.end.line + 1, 0);
    ...
    private static createDiagnosticObject(range: vscode.Range, suggest: string) {
        let diag = new vscode.Diagnostic(range, suggest, vscode.DiagnosticSeverity.Information);
    ...
    public async translation(editor: vscode.TextEditor) {
        ...
        const selectionText = utils.getSelectedText(editor.document, editor.selection);
        if (!selectionText) {
            return;
        }
    ...
    private async postTranslation(text: string) {
        ...
        const pattern = Translator.Config.get<any[]>('pattern') || [{ src: 'en', target: ['ja'] }, { src: 'ja', target: ['en'] }];
            ...
            if (results && results.status === 0) {
                ...
                if (isSource) {
                    ...
                    if (next && next.target[0]) {
                        ...
                        return await this.requestWithTimeout(text, next.target[0]).then(results => {
                            if (results.status === 0) {
                                // Translated correctly.
                                return this.parsTranslated(results.res, results.res.src, next.target[0]);
                            } else {
    ...
    private async requestWithTimeout(text: string, target: string) {
        ...
        const processingTimeout = Translator.Config.get<number>('processingTimeout', 750);

        return await Promise.race([request(options), new Promise((_, reject) => setTimeout(reject, processingTimeout))]).then(
            async res => {
                if (res.src) {
    ...
    activate() {
        ...
        this._addOnSuggestionCommand = vscode.commands.registerCommand(Translator.AddOnSuggestion, this.addWithSuggestion.bind(this));
        ...
        this._diagnostics = vscode.languages.createDiagnosticCollection(Translator.Source);
