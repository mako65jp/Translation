...
export function activate(context: vscode.ExtensionContext) {
  ...
  const translateToJapanese = vscode.commands.registerTextEditorCommand(
    'extension.translateToJapanese',
    async (editor, edit) => {
  ...
  const translateToEnglish = vscode.commands.registerTextEditorCommand(
...
async function executeTranslation(
  ...
  return await request(options).then(
    async (res: { src: string; sentences: [] }) => {
...
// Generate translation site parameters.
function getParameterForTranslateSite(
  text: string,
  source: string,
  target: string
) {
